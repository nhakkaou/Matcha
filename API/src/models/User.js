...
Registration(user){
        return new Promise ((resolve, reject ) => (
          ...
          .then((rows) =>{
            if (isEmpty(rows)) throw new Error('An error occured. Please try again later.')
              const html ='<h1>Welcome!</h1><p> Please click here: </p><a href="http://localhost:5000/confirm?token='+this.user.Registertoken+'">here</a>'
    ...
    EditProfile(firstname, lastname, username, password, email, birthday, gender, biography, tags, profileComplete, latitude, longitude, age, orientation, id, tags_new){
       ...
       Reset(email){
        ...
        return new Promise ((resolve, reject) => {
          ...
          .then(() => {
            if(s == 1){
          let html = '<h1>Welcome!</h1>To change your password click in the link below! Link<p> Please click here: </p><a href="http://localhost:5000/fgpassword?token='+tk+'">here</a>'
